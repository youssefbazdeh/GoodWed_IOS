//
//  updateChecklistView.swift
//  goodwed
//
//  Created by omarKaabi on 28/4/2023.
//

import SwiftUI
import Photos

struct updateChecklistView: View {
    @Environment(\.presentationMode) var presentationMode
        
    let selecctedID: String
    ...
    init(selectedID: String,selectedName: String, selectedType: String, selectedNote: String, selectedImage: String, selectedDate: Date, selectedStatus: String) {
        self.selecctedID = selectedID
        ...
        print(selectedImage)
        _nom = State(initialValue: selectedName)
        _type = State(initialValue: selectedType)
        _note = State(initialValue: selectedNote)
        _deadline = State(initialValue: selectedDate)
        _selectedOption = State(initialValue: selectedStatus)
        // ...
    }
            ...
            VStack(alignment: .leading) {
                ...
                VStack{
                    Button(action: {
                        ...
                        let request = ChecklistRequest(nom: nom, type: type, note: note,image: selectedImage, date: dateString, status: selectedOption)
                        print(request)
                        checklistViewModel.UpdateChecklist(request: request,id: selecctedID) { result in
                            switch result {
    ...
    struct updateChecklistView_Previews: PreviewProvider {
        static var previews: some View {
            updateChecklistView(selectedID: "",selectedName: "", selectedType: "", selectedNote: "", selectedImage: "", selectedDate: Date(), selectedStatus: "")
        }
    }
}
